    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-stone-500 text-sm font-medium">Loading batch status…</p>
        </div>
      </div>
    )
  }

  if (!data) return <div className="text-stone-500 text-sm">Batch not found.</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-xl font-medium text-app-text tracking-tight">Batch Results</h1>
          <p className="text-sm text-app-text-dim mt-0.5">
            {data.processedRows} of {data.totalRows} rows processed
            {isActive && (
              <span className="ml-1.5 text-teal-500">· updating every 2.5s</span>
            )}
          </p>
        </div>
      ...
      {/* Processing Banner */}
      {isActive && (
        <div className="rounded-xl border border-teal-200 bg-gradient-to-br from-teal-50 to-teal-50/50 px-5 py-4 animate-fade-up">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-4 mb-3">
                <span className="text-sm font-semibold text-teal-800">Processing in progress</span>
                <span className="text-sm font-bold text-teal-700 tabular-nums">{pct}%</span>
              </div>

              {/* Progress bar with shimmer */}
              <div
                className="w-full bg-teal-100 rounded-full h-2 overflow-hidden"
                role="progressbar"
                aria-valuenow={pct}
                aria-valuemin={0}
                aria-valuemax={100}
              ...
              >
                <div
                  className="h-2 rounded-full bg-teal-500 transition-all duration-700 ease-out shimmer-bar"
...
function ResultRow({
  ...
  const borderAccent = item.matchTier
    ? (tierBorderColor[item.matchTier as keyof typeof tierBorderColor] ?? 'border-l-stone-200')
  ...
  return (
    <div
      className={`bg-white rounded-xl border border-stone-200 border-l-4 ${borderAccent} shadow-sm p-5 space-y-4 hover:shadow-md transition-all duration-200 animate-fade-up ${staggerClass}`}
    >
      {/* Row header */}
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span className="text-xs font-bold text-stone-400 tracking-wider">ROW {item.rowNumber}</span>
        ...
        <div className="shrink-0 text-right">
          {item.confidenceScore !== null ? (
            <>
              ...
              {/* Mini score bar */}
              <div className="w-16 h-1 bg-stone-100 rounded-full mt-1.5 ml-auto overflow-hidden">
                <div
            ...
            <div className="text-stone-300 text-2xl font-bold">—</div>
          )}
        </div>
      </div>
      ...
      {item.status === 'processing' && !item.selectedCandidate && (
        <div className="space-y-2 py-1">
          <div className="h-3 bg-stone-100 rounded-full w-3/4 shimmer-bar" />
          <div className="h-3 bg-stone-100 rounded-full w-1/2 shimmer-bar" />
        </div>
      )}
      ...
      {item.status === 'pending' && !item.selectedCandidate && (
        <div className="space-y-2 py-1">
          <div className="h-3 bg-stone-100 rounded-full w-2/3 shimmer-bar" />
          <div className="h-3 bg-stone-100 rounded-full w-2/5 shimmer-bar" />
        </div>
      )}
      ...
      {item.selectedCandidate && (
        <Link
          ...
          className="flex items-start justify-between gap-4 rounded-lg border border-stone-200 bg-stone-50 hover:bg-white hover:border-teal-300 hover:shadow-sm p-4 transition-all group"
        >
          <div className="min-w-0">
            ...
            {item.selectedCandidate.domain && (
              <div className="text-xs text-stone-500 mt-0.5">{item.selectedCandidate.domain}</div>
            )}
            {item.selectedCandidate.sourceProviders.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2.5">
                {item.selectedCandidate.sourceProviders.map((provider) => (
                  <span key={provider} className="text-xs px-2 py-0.5 rounded-full bg-white border border-stone-200 text-stone-600">
      ...
      {item.matchTier === 'suggested' && item.suggestedCandidates.length > 0 && item.resolutionInputId && (
        <div>
          <p className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-2.5">Top candidates</p>
            ...
            {item.suggestedCandidates.map((candidate) => (
              ...
              >
                <div>
                  ...
                  {candidate.domain && (
                    <div className="text-xs text-stone-500 mt-0.5">{candidate.domain}</div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {candidate.sourceProviders.map((provider) => (
                    <span key={provider} className="text-xs px-2 py-0.5 rounded-full bg-stone-100 text-stone-600">
                      {provider.replace(/_/g, ' ')}
                    </span>
                  ))}
                </div>
                <button
                  ...
                  className="w-full text-xs font-semibold rounded-lg border px-3 py-1.5 transition-colors
                    ...
                    border-teal-200 text-app-accent hover:bg-teal-50 active:bg-teal-100"
                >
